...
class ContinuousMessagePlugin(Star):
    ...
    async def handle_private_msg(self, event: AstrMessageEvent):
        ...
        # 1. 解析消息内容
        raw_text, has_image, current_urls, current_link_contexts = self.parser.parse_message(
            event.message_obj
        )
        if not raw_text:
        ...
        # 3. 忽略空消息
        if not raw_text and not has_image and not current_link_contexts:
            return
        ...
        if uid in self.sessions:
            ...
            session['items'].append({
                ...
                'text': raw_text,
                'images': current_urls,
                'link_context_urls': current_link_contexts,
            })
            if raw_text:
                session['buffer'].append(raw_text)
            if current_urls:
                session['images'].extend(current_urls)
            if current_link_contexts:
                session['link_context_urls'].extend(current_link_contexts)
        ...
        self.sessions[uid] = {
            ...
            'link_context_urls': list(current_link_contexts),
            'items': [first_item],
        ...
        link_context_candidates = list(dict.fromkeys(session_data.get('link_context_urls', [])))
        original_image_count = len(all_images)
        merged_text = self.merge_separator.join(buffer).strip()
        merged_text, all_images = await self.link_parser.enrich(
            merged_text,
            all_images,
            parse_candidates=link_context_candidates,
        )
        ...
        self.parser.reconstruct_event(
            ...
            merged_text,
            all_images,
            link_context_urls=link_context_candidates,
        )
        return
