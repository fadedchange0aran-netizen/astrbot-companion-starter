...
**v2.4.0 更新**：
  ...
  新增撤回消息过滤功能，在防抖等待期间若用户撤回了最新一条消息（比如连续发送“1”、“2”、“3”，撤回“3”，则“3”不会发送给llm，但是若继续撤回“2”则捕捉不到新的撤回事件），插件会自动将其从待合并队列中移除；若所有消息均被撤回则不会向 LLM 发送任何内容（仅 aiocqhttp / OneBot v11 平台有效）。
...
### enable_recall_filter（启用撤回消息过滤）
- **类型**：布尔值
- **默认值**：`true`
- **说明**：在防抖等待期间，若用户撤回了某条消息，插件会将其从待合并队列中移除。若所有消息均被撤回，则不会向 LLM 发送任何内容
- **平台支持**：仅 aiocqhttp / OneBot v11 平台有效
- **提示**：关闭后撤回操作不会影响已进入防抖队列的消息
...
### max_typing_wait（输入状态超时保护）
- **类型**：浮点数（秒）
- **默认值**：`60`
- **说明**：用户正在输入时的最大等待时间。超过此时间未收到新的输入状态通知，将自动恢复结算
- **作用**：防止平台未发送停止输入事件，或因网络原因导致会话长时间卡住

### enable_qq_card_parsing（启用 QQ 卡片解析）
- **类型**：布尔值
- **默认值**：`true`
- **说明**：是否从 QQ `json` 卡片中提取原始链接，并将结果写入合并消息
- **补充**：
  - 关闭后，QQ 卡片中的链接不会被提取
  - 关闭后不会影响普通消息合并、输入状态感知、转发提取等其他功能

### qq_card_disabled_platforms（QQ 卡片解析屏蔽平台）
- **类型**：列表
- **默认值**：`[]`
- **说明**：用于屏蔽指定平台的 QQ 卡片链接提取
- **可选平台内部名**：
  - `bilibili`：B站
  - `xhs`：小红书
  - `xiaoheihe`：小黑盒
...
### qq_card_prompt（QQ 卡片解析引导词）
- **类型**：字符串
- **默认值**：`"[卡片链接]"`
- **说明**：每条提取出的 QQ 卡片链接前附加的引导词
- **示例输出**：
  - 默认：`[卡片链接] https://www.xiaohongshu.com/discovery/item/xxx`
  - 自定义为 `"[分享链接]"` 后：`[分享链接] https://www.xiaohongshu.com/discovery/item/xxx`
  - 留空时：仅输出链接本身

### enable_link_parsing（启用链接解析增强）
- **类型**：布尔值
- **默认值**：`true`
- **说明**：自动识别合并文本中的链接，并为支持的平台补充标题、正文、图片、封面、作者等信息
- **补充**：
  - 关闭后仍可保留 QQ 卡片链接提取
  - 关闭后不会追加 `[链接解析]` 或 `[链接解析失败]` 分节

### link_parser_disabled_platforms（链接解析屏蔽平台）
- **类型**：列表
- **默认值**：`[]`
- **说明**：用于屏蔽指定平台的链接内容解析
- **可选平台内部名**：
...
### link_parser_success_prompt（链接解析成功引导词）
- **类型**：字符串
- **默认值**：`"[链接解析]"`
- **说明**：当至少有一个链接解析成功时，写入合并消息中的分节标题
- **示例输出**：
  ```text
  [链接解析]
  平台: 小红书
...
### link_parser_failure_prompt（链接解析失败引导词）
- **类型**：字符串
- **默认值**：`"[链接解析失败]"`
- **说明**：当匹配到链接但解析失败时，写入合并消息中的失败分节标题
- **示例输出**：
  ```text
  [链接解析失败]
  平台: 小红书
  链接: https://www.xiaohongshu.com/discovery/item/xxx
  原因: ...
  ```

### link_parser_max_links（单次最多解析链接数）
- **类型**：整数
- **默认值**：`3`
- **说明**：一次合并结算中最多解析多少个链接
- **建议值**：
...
### link_parser_timeout（链接解析超时）
- **类型**：浮点数（秒）
- **默认值**：`12.0`
- **说明**：单个链接解析请求的总超时时间
- **建议**：
  - 网络良好时可适当调低
  - 如果常用代理或目标站点较慢，可以适当调高

### link_parser_merge_images（合并解析出的图片）
- **类型**：布尔值
- **默认值**：`true`
- **说明**：是否把链接解析得到的图片或视频封面并入最终发送消息的图片列表
- **补充**：
  - 开启后，LLM 可以同时拿到解析出的配图或封面
  - 关闭后，图片仍会出现在解析文本中对应的数量统计里，但不会并入图片消息流

### link_parser_max_text_length（正文截断长度）
- **类型**：整数
- **默认值**：`600`
- **说明**：解析出的正文最多保留多少字符，避免附加上下文过长
- **建议**：
  - 内容摘要为主时可设置得更短
  - 需要更完整上下文时可适当调高

### link_parser_proxy（链接解析代理）
- **类型**：字符串
- **默认值**：`""`
- **说明**：可选代理地址，支持 `HTTP` 或 `HTTPS`
