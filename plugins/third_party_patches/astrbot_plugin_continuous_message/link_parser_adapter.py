...
class LinkParserAdapter:
    ...
    async def enrich(
        ...
        text: str,
        image_urls: list[str],
        parse_candidates: list[str] | None = None,
    ) -> tuple[str, list[str]]:
        if (not text and not parse_candidates) or self.max_links <= 0:
        ...
        candidate_text = "\n".join(
            str(item or "").strip() for item in (parse_candidates or []) if str(item or "").strip()
        )
        search_text = text
        if candidate_text:
            search_text = f"{text.strip()}\n{candidate_text}".strip() if text else candidate_text

        matches = self.service.find_matches(search_text, self.max_links)
        if not matches:
