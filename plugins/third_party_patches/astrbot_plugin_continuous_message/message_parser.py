...
class MessageParser:
    ...
    def _extract_urls_from_json_payload(self, payload) -> List[str]:
        ...
        deduped = self._dedupe_keep_order(extracted)
        non_wrappers = [u for u in deduped if not self._is_wrapper_share_url(u)]
        return non_wrappers or deduped

    def _extract_link_context_urls_from_json_payload(self, payload) -> List[str]:
        """提取供下游链接理解使用的候选链接，不受前置展示屏蔽影响。"""
        payload = self._safe_json_loads(payload, source="qq_card_root")
        if isinstance(payload, dict) and "data" in payload:
            payload["data"] = self._safe_json_loads(payload.get("data"), source="qq_card_data")
            ...
            if isinstance(node, dict):
                for key, val in node.items():
                    ...
                    if key_lower in self._URL_KEY_HINTS and isinstance(val, str):
                        normalized = self._normalize_url(val)
                        if normalized:
                            canonical = self._canonicalize_known_share_url(normalized)
                            extracted.append(canonical or normalized)
                    walk(val)
            elif isinstance(node, list):
                for item in node:
                    walk(item)
        ...
        walk(payload)
        return self._dedupe_keep_order(extracted)
    ...
    def parse_message(self, message_obj) -> Tuple[str, bool, List[str], List[str]]:
        """
        解析消息对象，提取文本、图片和合并转发信息
        
        Returns:
            (文本内容, 是否包含图片, 图片URL列表, 下游链接理解候选)
        """
        text = ""
        has_image = False
        image_urls = []
        ...
        try:
            ...
            for component in message_obj.message:
                ...
                if json_payload is not None:
                    try:
                        local_card_urls = self._extract_urls_from_json_payload(json_payload)
                        local_link_context_urls = self._extract_link_context_urls_from_json_payload(
                            json_payload
                        )
                        if any(
                            self._identify_card_platform(url) in self.qq_card_disabled_platforms
                            for url in local_link_context_urls
                        ):
                            local_card_urls = [
                                url for url in local_card_urls if not self._is_wrapper_share_url(url)
                            ]
                        card_urls.extend(local_card_urls)
                        link_context_urls.extend(local_link_context_urls)
                    except Exception as exc:
        ...
        return text, has_image, image_urls, self._dedupe_keep_order(link_context_urls)
    ...
    def reconstruct_event(
        ...
        text: str,
        image_urls: List[str],
        link_context_urls: List[str] | None = None,
    ):
        """
        重构消息事件，将合并后的文本和图片重新组装到事件对象中
        这样事件可以继续传播给后续的插件/框架处理
        """
        if link_context_urls:
            try:
                event._aran_link_context_candidates = self._dedupe_keep_order(link_context_urls)
            except Exception:
                pass
        ...
        if hasattr(event, "message_obj"):
            if not hasattr(event, "_aran_original_message_chain") and hasattr(event.message_obj, "message"):
                try:
                    event._aran_original_message_chain = list(event.message_obj.message or [])
                except Exception:
                    pass
            if not hasattr(event, "_aran_original_raw_message") and hasattr(event.message_obj, "raw_message"):
                try:
