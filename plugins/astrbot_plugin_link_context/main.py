...
SUPPORTED_PLATFORMS = {"bilibili", "xiaohongshu", "weibo", "douyin", "twitter"}
...
@register(
    PLUGIN_NAME,
    "Aran",
    "给机器人提供 B站/小红书/微博/抖音/Twitter(X) 链接理解，以及最近一次工具成功结果感知。",
    "0.1.0",
)
class LinkContextPlugin(Star):
    ...
    def _get_parser_manager(self):
        ...
        if not parsers:
            raise RuntimeError("media_parser 中没有可用的 B站/小红书/微博/抖音/Twitter(X) 解析器")
    ...
    async def read_link_context(self, event: AstrMessageEvent, text_or_url: str):
        """
        读取 B站、小红书、微博、抖音或 Twitter(X) 链接的标题、作者、正文摘要和热评，让你可以在同一条回复里继续讨论。
        ...
        if not summary:
            return "没有识别到可解析的 B站、小红书、微博、抖音或 Twitter(X) 链接。"
